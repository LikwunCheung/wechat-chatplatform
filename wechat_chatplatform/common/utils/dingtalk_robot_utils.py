...
def send_new_applier_message(anchor):
    ...
    text = text.format(anchor.avatar, anchor.name, anchor.nickname, anchor.birthday,
                       Gender.GenderChoices.value[anchor.gender][1], anchor.city_id, anchor.wechat_id, anchor.identity)
    for admin_user in admin_users:
        if admin_user['dingtalk_robot']:
            resp = dingtalk_robot_handler.send_action_card(token=admin_user['dingtalk_robot'], title=title, text=text, btns=btns)
